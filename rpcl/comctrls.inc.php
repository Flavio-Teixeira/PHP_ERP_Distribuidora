...
class ListColumn extends Persistent
{
        ...
        * values are not transfered to the RPCL framework. They are available
        * on client side only.
...
class CustomTreeView extends QWidget
{
        ...
        * of the object inspector of RadPHP XE. It may also be used to convert
...
//                echo "  $this->Name.setTop(0);\n";
        ...
        function dumpJsEvents()
        {
                ...
                $this->dumpJSEvent($this->_jsontreeclose);
                $this->dumpJSEvent($this->_jsontreeopenwhileempty);
                $this->dumpJSEvent($this->_jsontreeopenwithcontent);
        }
...
class GraphicMainMenu extends Control
{
        ...
        function dumpHeaderCode()
        {
                ...
                if (!defined('DYNAPI_'.strtoupper($this->className())))
                {
                        ...
                        if (!defined('DYNAPI'))
                        {
                                echo "dynapi.library.setPath('".RPCL_HTTP_PATH."/dynapi/src/');\n";
...
?>
  ...
  _editor_url  = "<?php echo RPCL_HTTP_PATH;      ?>/resources/xinha/";
  _editor_lang = "en";      // And the language we need to use in the editor.
  </script>

  <script type="text/javascript" src="<?php echo RPCL_HTTP_PATH; ?>/resources/xinha/htmlarea.js"></script>
<?php
                                }
?>
...
class TrackBar extends Control
{
        ...
        function dumpHeaderCode()
        {
                if (($this->ControlState & csDesigning)==csDesigning)
                {
					echo "<script type=\"text/javascript\" src=\"".RPCL_HTTP_PATH."/js/common.js\"></script>\n";
                }
                if (!defined('SLIDERBAR'))
                {
                	define('SLIDERBAR', 1);
					echo '<script type="text/javascript" src="'. RPCL_HTTP_PATH .'/slider/js/range.js"></script>';
					echo '<script type="text/javascript" src="'. RPCL_HTTP_PATH .'/slider/js/timer.js"></script>';
					echo '<script type="text/javascript" src="'. RPCL_HTTP_PATH .'/slider/js/slider.js"></script>';

					echo '<link type="text/css" rel="StyleSheet" href="'. RPCL_HTTP_PATH .'/slider/css/luna/luna.css" />';
                }
        }
