...
//Class definition
class relcontaspagarefetivo extends Page
{
   public $opcao_soma_dia = null;
   ...
   var campo = document.relcontaspagarefetivo.mgt_data_final
   ...
   var campo = document.relcontaspagarefetivo.mgt_data_inicial
   var digits="0123456789/"
   var campo_temp
   for (var i=0;i<campo.value.length;i++){
       campo_temp=campo.value.substring(i,i+1)
...
?>
   ...
   var campo = document.relcontaspagar.mgt_pedido_data_final
   var digits="0123456789"
   var campo_temp
   for (var i=0;i<campo.value.length;i++){
       campo_temp=campo.value.substring(i,i+1)
...
?>
   ...
   {
     document.relcontaspagarefetivo.bt_imprimir.focus();
     return false;
   }
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.relcontaspagarefetivo.mgt_data_final.focus();
     return false;
   }
...
?>
   ...
   var campo = document.relcontaspagar.mgt_periodo_data_inicial
   var digits="0123456789"
   var campo_temp
   for (var i=0;i<campo.value.length;i++){
       campo_temp=campo.value.substring(i,i+1)
...
<?php
   ...
   {
      if((trim($this->mgt_data_inicial->Text) != '')And(trim($this->mgt_data_final->Text) != ''))
      {
         $this->MSG_Erro->Caption = '';
      }

      if(trim($this->mgt_data_inicial->Text) == '')
      {
         $this->MSG_Erro->Caption = 'Por favor, informe a Data Inicial';
      }

      else if(trim($this->mgt_data_final->Text) == '')
      {
      ...
      if(trim($this->MSG_Erro->Caption) == '')
      {
         ...
         $_SESSION['opcao_soma_dia'] = $this->opcao_soma_dia->ItemIndex;
         $_SESSION['conta_padrao'] = $this->conta_padrao->ItemIndex;

         echo "<script language=\"JavaScript\">";
         echo "window.open('rel_contas_pagar_efetivo_imp.php','rel_contas_pagar_efetivo_imp','toolbar=yes,location=no,directories=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,maximized=no,fullscreen=no,width=775,innerWidth=775,offsetWidth=775,clientWidth=775,pageWidth=775,height=550,innerHeight=550,offsetHeight=550,clientHeight=550,pageHeight=550');";
         echo "</script>";
      }
   }

   function relcontaspagarefetivoCreate($sender, $params)
   {
      ...
      $this->conta_padrao->Clear();

      $this->conta_padrao->AddItem('--- Todas as Contas ---', null , 0);

      $Comando_SQL = "select * from mgt_contas_bancarias order by mgt_conta_bancaria_banco";
      ...
      if((GetConexaoPrincipal()->SQL_MGT_Contas->EOF) != 1)
      {
         ...
         {
            $this->conta_padrao->AddItem(GetConexaoPrincipal()->SQL_MGT_Contas->Fields['mgt_conta_bancaria_banco'], null , GetConexaoPrincipal()->SQL_MGT_Bancos->Fields['mgt_conta_bancaria_nro']);
   ...
   function relcontaspagarefetivoJSLoad($sender, $params)
   {
...
global $relcontaspagarefetivo;

//Creates the form
$relcontaspagarefetivo = new relcontaspagarefetivo($application);

//Read from resource file
$relcontaspagarefetivo->loadResource(__FILE__);

//Shows the form
$relcontaspagarefetivo->show();
