...
class Persistent extends Object
{
        ...
        */
        ...
        //get_class_vars doesn't work well in new PHP versions, for the purposes of this
        public function __sleep()
        {
          ...
          foreach($vars as $key=>$value)
          {
            if ($key[0]=='_')
            {
              if ($this->methodExists('set'.substr($key,1))) $result[]=$key;
            }
            if ($key=='owner') $result[]=$key;
          }
          return($result);
        }
        */

        function newserialize()
        {
          $_SESSION[$this->readNamePath()]=serialize($this);
        }

        function newunserialize($source)
        {
          $cname=get_class($this);
          $start='O:'.strlen($cname).':"'.$cname.'"';
          $source='a'.substr($source,strlen($start));
          $props=unserialize($source);

          foreach($props as $prop=>$value)
          {
            if ($prop=='owner') continue;
            $prop=str_replace(chr(0).'*'.chr(0).'_','*set',$prop);
            if ($prop[0]=='_') $method='set'.substr($prop,1);
            else $method=substr($prop,1);

            if ($this->methodExists($method))
            {
              if ((is_object($value)) && ($value->inheritsFrom('Persistent')))
              {
                if ($value->inheritsFrom('Component'))
                {
                  $propvalue=$value->_name;
                  if (is_object($value->owner))
                  {
                    if ($value->owner!=$this->owner)
                    {
                      $propvalue=$value->owner->name.'.'.$propvalue;
                    }
                  }
                ...
                {
                  $persistentprop='g'.substr($prop,2);
                  $objectproperty=$this->$persistentprop();
                  $objectproperty->assign($value);
                }
              }
  ...
  */
   ...
   * is a getter method) and retrieve those stored by a previous call to {@link serialize()}.
   *
...
class Component extends Persistent
{
        ...
        *
        * It initializes the component, and saves its {@link $aowner owner} and children components.
        *
        ...
        * @link wiki://Component_Writer%27s_Guide_::_Execution_Order
        * @internal
        *
        */
        function loaded()
        ...
        */
         ...
         *
         * @link wiki://Component_Writer%27s_Guide_::_Adding_events
         *
         ...
         * to get updated.
         *
         * @link wiki://Component_Writer%27s_Guide_::_Ajax_Integration#Simple_Ajax_using_xajax
         *
        ...
        /**
         * Method called before {@link init()}.
         *
         * The streaming system calls this method before calling {@link init()}, so if you need to
        ...
        *
        * @link wiki://Component_Writer%27s_Guide_::_Data-aware_Controls
        ...
        */
         ...
         *
         * If you write components, override this method to dump all JavaScript code required by
        ...
        /**
         * Dumps header code required by this component.
         *
         * If you write components, override this method to dump all header code you need to make
         * your component work.
         *
         * @see dumpJavascript()
         *
