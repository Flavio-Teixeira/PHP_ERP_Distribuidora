	...
	<TR>
		...
		<TD WIDTH="717">
			<TABLE BORDER="0" WIDTH="717" CELLPADDING="0" CELLSPACING="0">
				...
				<TR>
					<TD WIDTH="717" HEIGHT="18" ALIGN="CENTER"><SPAN CLASS="LETRA15">(Vendas Programadas) Relatorio de Produtos Vendidos<?php if($tipo == "0"){ echo " por Cliente";}elseif($tipo == "1"){ echo " por Produto";}?></SPAN></TD>
				</TR>
...
?>
   ...
   if(trim($cfops) <> '')
   {
      $sql_clientes = mysql_query("
							...
							from
								mgt_clientes, mgt_programadas 
							where
								mgt_cliente_numero = mgt_programada_cliente_numero and
								mgt_programada_data >= '" . data_amd($data_inicial) . "' and
								mgt_programada_data <= '" . data_amd($data_final) . "' and
                mgt_programada_cfop IN (" . trim($cfops) . ")
   ...
   else
   {
      $sql_clientes = mysql_query("
							...
							where
								mgt_cliente_numero = mgt_programada_cliente_numero and
								mgt_programada_data >= '" . data_amd($data_inicial) . "' and
								mgt_programada_data <= '" . data_amd($data_final) . "'
							group by mgt_cliente_numero order by mgt_cliente_nome 
						");
   }
   ...
   for($ind1 = 0; $ind1 < mysql_num_rows($sql_clientes); $ind1++)
   {
      if(trim($cfops) <> '')
      {
         $sql_total_cliente = mysql_query("
								select sum(mgt_programada_produto_quantidade) as qtd_total, sum(mgt_programada_produto_valor_total) as valor_total
								from
									mgt_programadas, mgt_programadas_produtos
								where
									mgt_programada_data >= '" . data_amd($data_inicial) . "' and
									mgt_programada_data <= '" . data_amd($data_final) . "' and
                  mgt_programada_cfop IN (" . trim($cfops) . ") and
									mgt_programada_produto_numero_nota = mgt_programada_numero and
									mgt_programada_cliente_numero = '" . mysql_result($sql_clientes, $ind1, "mgt_cliente_numero") . "'
							");
      }
      else
      {
...
?>
      ...
      if(trim($cfops) <> '')
      {
         $sql_produtos = mysql_query("
								select mgt_programada_produto_codigo, mgt_programada_produto_descricao
								from
									mgt_clientes, mgt_programadas, mgt_programadas_produtos
								where
									mgt_programada_data >= '" . data_amd($data_inicial) . "' and
									mgt_programada_data <= '" . data_amd($data_final) . "' and
                  mgt_programada_cfop IN (" . trim($cfops) . ") and
									mgt_programada_numero = mgt_programada_produto_numero_nota and
									mgt_cliente_numero = mgt_programada_cliente_numero and
									mgt_programada_cliente_numero = '" . mysql_result($sql_clientes, $ind1, "mgt_cliente_numero") . "'
								group by mgt_programada_produto_codigo
							");
      }
      else
      {
      ...
      for($ind2 = 0; $ind2 < mysql_num_rows($sql_produtos); $ind2++)
      {
         if(trim($cfops) <> '')
         {
            $sql_total_produto = mysql_query("
                    ...
                    mgt_programada_cfop IN (" . trim($cfops) . ") and
										mgt_programada_produto_numero_nota = mgt_programada_numero and
										mgt_programada_produto_codigo = '" . mysql_result($sql_produtos, $ind2, "mgt_programada_produto_codigo") . "' and
										mgt_programada_cliente_numero = '" . mysql_result($sql_clientes, $ind1, "mgt_cliente_numero") . "'
								");
         }
         else
...
?>
									...
									<TD WIDTH="717">
										<TABLE BORDER="0" WIDTH="717" CELLPADDING="0" CELLSPACING="0">
											<TR>
												...
												<TD WIDTH="060" HEIGHT="13"><SPAN CLASS="LETRA10"><?php echo mysql_result($sql_produtos, $ind2, "mgt_programada_produto_codigo");   ?></SPAN></TD>
												<TD WIDTH="070" HEIGHT="13"><SPAN CLASS="LETRA10"><B>Descricao:</B></SPAN></TD>
												<TD WIDTH="227" HEIGHT="13"><SPAN CLASS="LETRA10"><?php echo mysql_result($sql_produtos, $ind2, "mgt_programada_produto_descricao");   ?></SPAN></TD>
									...
									<TD WIDTH="717">
										<TABLE BORDER="0" WIDTH="717" CELLPADDING="0" CELLSPACING="0">
											<TR>
												...
												<TD WIDTH="070" HEIGHT="13" ALIGN="CENTER"><SPAN CLASS="LETRA10"><B>Docto</B></SPAN></TD>
         ...
         if(trim($cfops) <> '')
         {
            $sql_notas = mysql_query("
									select mgt_programada_numero, mgt_programada_produto_quantidade, mgt_programada_produto_valor_total
									from
										mgt_programadas, mgt_programadas_produtos
									where
										mgt_programada_data >= '" . data_amd($data_inicial) . "' and
                    ...
                    mgt_programada_cfop IN (" . trim($cfops) . ") and
										...
										mgt_programada_produto_numero_nota = mgt_programada_numero and
										mgt_programada_cliente_numero = '" . mysql_result($sql_clientes, $ind1, "mgt_cliente_numero") . "'
									order by mgt_programada_produto_valor_total desc
								");
         }
         else
         {
										...
										<TD WIDTH="717">
											<TABLE BORDER="0" WIDTH="717" CELLPADDING="0" CELLSPACING="0">
												<TR>
													...
													<TD WIDTH="070" HEIGHT="13" ALIGN="CENTER"><SPAN CLASS="LETRA10"><?php echo mysql_result($sql_notas, $ind3, "mgt_programada_numero");    ?></SPAN></TD>
													<TD WIDTH="070" HEIGHT="13" ALIGN="CENTER"><SPAN CLASS="LETRA10"><?php echo mysql_result($sql_notas, $ind3, "mgt_programada_produto_quantidade");    ?></SPAN></TD>
													<TD WIDTH="120" HEIGHT="13" ALIGN="CENTER"><SPAN CLASS="LETRA10"><?php echo mysql_result($sql_notas, $ind3, "mgt_programada_produto_valor_total");    ?></SPAN></TD>
												</TR>
											</TABLE>
										</TD>
									</TR>
   ...
   if(trim($cfops) <> '')
   {
      $sql_produtos = mysql_query("
							...
							where
								...
								mgt_programada_data <= '" . data_amd($data_final) . "' and
                mgt_programada_cfop IN (" . trim($cfops) . ") and
								mgt_programada_numero = mgt_programada_produto_numero_nota
							group by mgt_programada_produto_codigo
						");
   }
   else
   ...
   for($ind1 = 0; $ind1 < mysql_num_rows($sql_produtos); $ind1++)
   {
      if(trim($cfops) <> '')
      {
         $sql_total_produto = mysql_query("
                  ...
                  mgt_programada_cfop IN (" . trim($cfops) . ") and
									mgt_programada_produto_numero_nota = mgt_programada_numero and
									mgt_programada_produto_codigo = '" . mysql_result($sql_produtos, $ind1, "mgt_programada_produto_codigo") . "'
							");
      }
      else
      {
							...
							<TR>
								<TD WIDTH="717">
									<TABLE BORDER="0" WIDTH="717" CELLPADDING="0" CELLSPACING="0">
										<TR>
											<TD WIDTH="100" HEIGHT="13"><SPAN CLASS="LETRA10"><B>Produto Codigo:</B></SPAN></TD>
											<TD WIDTH="060" HEIGHT="13"><SPAN CLASS="LETRA10"><?php echo mysql_result($sql_produtos, $ind1, "mgt_programada_produto_codigo");  ?></SPAN></TD>
											<TD WIDTH="070" HEIGHT="13"><SPAN CLASS="LETRA10"><B>Descricao:</B></SPAN></TD>
											<TD WIDTH="227" HEIGHT="13"><SPAN CLASS="LETRA10"><?php echo mysql_result($sql_produtos, $ind1, "mgt_programada_produto_descricao");  ?></SPAN></TD>
											<TD WIDTH="050" HEIGHT="13" ALIGN="RIGHT"><SPAN CLASS="LETRA10"><B>Totais:&nbsp;</B></SPAN></TD>
											<TD WIDTH="100" HEIGHT="13"><SPAN CLASS="LETRA10"><B>Qtde: </B><?php echo mysql_result($sql_total_produto, 0, "qtd_total");  ?></SPAN></TD>
											<TD WIDTH="110" HEIGHT="13"><SPAN CLASS="LETRA10"><B>Valor: </B><?php echo mysql_result($sql_total_produto, 0, "valor_total");  ?></SPAN></TD>
										</TR>
         ...
         if(trim($cfops) <> '')
         {
            $sql_total_cliente = mysql_query("
                    ...
                    mgt_programada_cfop IN (" . trim($cfops) . ") and
										mgt_cliente_numero = mgt_programada_cliente_numero and
										mgt_programada_produto_numero_nota = mgt_programada_numero and
										mgt_programada_produto_codigo = '" . mysql_result($sql_produtos, $ind1, "mgt_programada_produto_codigo") . "' and
         ...
         if(trim($cfops) <> '')
         {
            $sql_notas = mysql_query("
                    ...
                    mgt_programada_cfop IN (" . trim($cfops) . ") and
										mgt_programada_produto_codigo = '" . mysql_result($sql_produtos, $ind1, "mgt_programada_produto_codigo") . "' and
										mgt_programada_produto_numero_nota = mgt_programada_numero and
										mgt_programada_cliente_numero = '" . mysql_result($sql_clientes, $ind2, "mgt_cliente_numero") . "'
									order by mgt_programada_produto_valor_total desc
								");
         }
         else
