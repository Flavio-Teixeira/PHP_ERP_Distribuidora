...
//Class definition
class cadservicos extends Page
{
   public $Label5 = null;
   public $Frame1 = null;
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.cadservicos.bt_buscar.focus();
     return false;
   }
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.cadservicos.opcao_busca.focus();
     return false;
   }
   ...
   function bt_cadastroClick($sender, $params)
   {
      ...
      {
         $Comando_SQL = "select * from mgt_servicos where mgt_servico_codigo = '" . $this->abrir_cadastro->Text . "' order by mgt_servico_codigo";
         ...
         {
            redirect('cad_servicos_alt.php?mgt_servico_codigo=' . $this->abrir_cadastro->Text );
         }
      }
   }
...
?>
   ...
   document.cadservicos.abrir_cadastro.value = registros.getTableModel().getValue(0, registros.getFocusedRow());
   ...
   function bt_buscarClick($sender, $params)
   {
      ...
      else
      {
         ...
         {
            $Comando_SQL = "select * from mgt_servicos where mgt_servico_codigo = '" . trim($this->dados_busca->Text) . "' order by mgt_servico_codigo";
         }
         elseif(trim($this->opcao_busca->ItemIndex) == "Descricao")
         {
            $Comando_SQL = "select * from mgt_servicos where mgt_servico_descricao like '%" . trim($this->dados_busca->Text) . "%' order by mgt_servico_codigo";
         }
      }
      ...
      $_SESSION['comando_sql_grid'] = GetConexaoPrincipal()->SQL_MGT_Servicos->SQL;
   }

   function cadservicosCreate($sender, $params)
   {
   ...
   function cadservicosJSLoad($sender, $params)
   {
...
//Creates the form
$cadservicos = new cadservicos($application);

//Read from resource file
$cadservicos->loadResource(__FILE__);
