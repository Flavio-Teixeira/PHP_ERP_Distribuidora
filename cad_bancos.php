...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.cadbancos.bt_buscar.focus();
     return false;
   }
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.cadbancos.opcao_busca.focus();
     return false;
   }
   ...
   function bt_cadastroClick($sender, $params)
   {
      ...
      {
         $Comando_SQL = "select * from mgt_bancos where mgt_banco_codigo = '" . $this->abrir_cadastro->Text . "' order by mgt_banco_codigo";
         ...
         {
            redirect('cad_bancos_alt.php?mgt_banco_codigo=' . $this->abrir_cadastro->Text );
         }
      }
   }
...
?>
   ...
   document.cadbancos.abrir_cadastro.value = registros.getTableModel().getValue(0, registros.getFocusedRow());
   ...
   function bt_buscarClick($sender, $params)
   {
      ...
      else
      {
         ...
         {
            $Comando_SQL = "select * from mgt_bancos where mgt_banco_codigo = '" . trim($this->dados_busca->Text) . "' order by mgt_banco_codigo";
         }
         elseif(trim($this->opcao_busca->ItemIndex) == "Descricao")
         {
            $Comando_SQL = "select * from mgt_bancos where mgt_banco_descricao like '%" . trim($this->dados_busca->Text) . "%' order by mgt_banco_codigo";
         }
      }
      ...
      $_SESSION['comando_sql_grid'] = GetConexaoPrincipal()->SQL_MGT_Bancos->SQL;
   }

   function cadbancosCreate($sender, $params)
   {
   ...
   function cadbancosJSLoad($sender, $params)
   {
...
//Creates the form
$cadbancos = new cadbancos($application);

//Read from resource file
$cadbancos->loadResource(__FILE__);
