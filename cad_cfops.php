...
?>
   ...
   {
     document.cadcfops.bt_cadastro.focus();
     return false;
   }
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.cadcfops.bt_buscar.focus();
     return false;
   }
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.cadcfops.opcao_busca.focus();
     return false;
   }
   ...
   function bt_cadastroClick($sender, $params)
   {
      ...
      {
         $Comando_SQL = "select * from mgt_cfops where mgt_cfop_numero = '" . $this->abrir_cadastro->Text . "' order by mgt_cfop_numero";
         ...
         {
            redirect('cad_cfops_alt.php?mgt_cfop_numero=' . $this->abrir_cadastro->Text );
         }
      }
   }
...
?>
   ...
   document.cadcfops.abrir_cadastro.value = registros.getTableModel().getValue(0, registros.getFocusedRow());
   ...
   function bt_buscarClick($sender, $params)
   {
      ...
      {
         $Comando_SQL = "select * from mgt_cfops order by mgt_cfop_numero";
      }
      else
      {
         if(trim($this->opcao_busca->ItemIndex) == "Numero")
         {
            $Comando_SQL = "select * from mgt_cfops where mgt_cfop_numero = '" . trim($this->dados_busca->Text) . "' order by mgt_cfop_numero";
         }
         elseif(trim($this->opcao_busca->ItemIndex) == "Descricao")
         {
            $Comando_SQL = "select * from mgt_cfops where mgt_cfop_descricao like '%" . trim($this->dados_busca->Text) . "%' order by mgt_cfop_numero";
         }
      }
      ...
      $_SESSION['comando_sql_grid'] = GetConexaoPrincipal()->SQL_MGT_CFOP->SQL;
   }

   function cadcfopsCreate($sender, $params)
   {
   ...
   function cadcfopsJSLoad($sender, $params)
   {
...
//Creates the form
$cadcfops = new cadcfops($application);

//Read from resource file
$cadcfops->loadResource(__FILE__);
