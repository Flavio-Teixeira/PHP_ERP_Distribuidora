    ...
    */
    ...
    function defaultCharset() { return ""; }
        ...
        */
        ...
        function defaultDriverName()   { return $this->_drivername=""; }
        /**
      ...
      /**
      * Use this property to let the component translate localhost to 127.0.0.1
      *
      * On latest MySQL versions on Windows Vista/7 with IPv6 enabled, you can have problems
      * using localhost as Host. Setting this property to true, just translates localhost to 127.0.0.1
      *
      * @return boolean
      */
        ...
        function DoConnect()
        {
            ...
            if (($this->ControlState & csDesigning)!=csDesigning)
            {
                ...
                try
                {
                    ...
                    $host=$this->Host;
                    if ($this->_hosttranslation)
                    {
                      if (strtolower($host)=='localhost') $host='127.0.0.1';
                    }

                    if (($this->DriverName=='borland_ibase') || ($this->DriverName=='ibase')  || ($this->DriverName=='firebird'))
                    {
                        $result=$this->_connection->PConnect($this->DatabaseName,$this->UserName,$this->UserPassword);
                    }
                    else
                    {
                        $result=$this->_connection->Connect($host,$this->UserName,$this->UserPassword,$this->DatabaseName);
                    }
                }
                catch (Exception $e)
                {
                    DatabaseError("Cannot connect to database server:".$e->getMessage());
                }
            }
            return($result);
        }
    ...
    function InternalOpen()
    {
        if (($this->ControlState & csDesigning)!=csDesigning)
        {
            ...
            if ((trim($this->_limitstart)=='-1') && (trim($this->_limitcount)=='-1'))
            {
                ...
                {
                  $this->_rs=$this->Database->Execute($query,false);
                  if ($this->_fetchquery!='') $this->_rs=$this->Database->Execute($this->_fetchquery);
                }
            }
            else
            {
                ...
                {
                  $this->_rs=$this->Database->Execute($query,false);
                  if ($this->_fetchquery!='') $this->_rs=$this->Database->ExecuteLimit($this->_fetchquery,$limitcount,$limitstart);
                }
            }
