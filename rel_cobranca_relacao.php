...
//Class definition
class relcobrancarelacao extends Page
{
   ...
   var campo = document.relcobrancarelacao.mgt_data_final
   ...
   var campo = document.relcobrancarelacao.mgt_data_inicial
   var digits="0123456789/"
   var campo_temp
   for (var i=0;i<campo.value.length;i++){
       campo_temp=campo.value.substring(i,i+1)
...
?>
   ...
   {
     document.relcobrancarelacao.bt_imprimir.focus();
     return false;
   }
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.relcobrancarelacao.mgt_data_final.focus();
     return false;
   }
...
<?php
   ...
   function bt_imprimirClick($sender, $params)
   {
      ...
      if(trim($this->MSG_Erro->Caption) == '')
      {
         ...
         echo "window.open('rel_cobranca_relacao_imp.php','rel_cobranca_relacao_imp','toolbar=yes,location=no,directories=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,maximized=no,fullscreen=no,width=775,innerWidth=775,offsetWidth=775,clientWidth=775,pageWidth=775,height=550,innerHeight=550,offsetHeight=550,clientHeight=550,pageHeight=550');";
         echo "</script>";
      }
   }

   function relcobrancarelacaoCreate($sender, $params)
   {
   ...
   function relcobrancarelacaoJSLoad($sender, $params)
   {
...
global $relcobrancarelacao;

//Creates the form
$relcobrancarelacao = new relcobrancarelacao($application);

//Read from resource file
$relcobrancarelacao->loadResource(__FILE__);

//Shows the form
$relcobrancarelacao->show();
