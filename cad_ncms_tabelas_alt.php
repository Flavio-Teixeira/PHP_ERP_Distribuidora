...
//Class definition
class cadncmstabelasalt extends Page
{
...
?>
   ...
   {
     document.cadncmstabelasalt.bt_alterar.focus();
     return false;
   }
<?php
   ...
   function bt_simClick($sender, $params)
   {
      if(trim($this->mgt_operacao_cadastro_motivo->text) <> '')
      {
         ...
         $Comando_SQL = "delete from mgt_classificacoes_fiscais_numeros ";
         $Comando_SQL .= " where ";
         $Comando_SQL .= "mgt_classificacao_fiscal_numero_ncm='" . trim($this->mgt_classificacao_fiscal_numero_ncm->Text) . "'";
   ...
   function cadncmstabelasaltCreate($sender, $params)
   {
      ...
      $mgt_classificacao_fiscal_numero_ncm = $_REQUEST['mgt_classificacao_fiscal_numero_ncm'];

      $Comando_SQL = "select * from mgt_classificacoes_fiscais_numeros where mgt_classificacao_fiscal_numero_ncm = '" . trim($mgt_classificacao_fiscal_numero_ncm) . "' order by mgt_classificacao_fiscal_numero_ncm";

      GetConexaoPrincipal()->SQL_MGT_Classificacoes_Fiscais_Numeros->Close();
      GetConexaoPrincipal()->SQL_MGT_Classificacoes_Fiscais_Numeros->SQL = $Comando_SQL;
      GetConexaoPrincipal()->SQL_MGT_Classificacoes_Fiscais_Numeros->Open();

      $this->mgt_classificacao_fiscal_numero_ncm->Text = $mgt_classificacao_fiscal_numero_ncm;
      $this->mgt_classificacao_fiscal_numero_cnae->Text = GetConexaoPrincipal()->SQL_MGT_Classificacoes_Fiscais_Numeros->Fields['mgt_classificacao_fiscal_numero_cnae'];
      $this->mgt_classificacao_fiscal_numero_descricao->Text = GetConexaoPrincipal()->SQL_MGT_Classificacoes_Fiscais_Numeros->Fields['mgt_classificacao_fiscal_numero_descricao'];
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.cadncmstabelasalt.mgt_classificacao_fiscal_numero_descricao.focus();
     return false;
   }
...
<?php
   ...
   {
      if(trim($this->mgt_classificacao_fiscal_numero_cnae->Text) == "")
      {
      ...
      {
         $Comando_SQL = "update mgt_classificacoes_fiscais_numeros set ";
         $Comando_SQL .= "mgt_classificacao_fiscal_numero_cnae='" . trim($this->mgt_classificacao_fiscal_numero_cnae->Text) . "', ";
         $Comando_SQL .= "mgt_classificacao_fiscal_numero_descricao='" . trim($this->mgt_classificacao_fiscal_numero_descricao->Text) . "' where ";
         $Comando_SQL .= "mgt_classificacao_fiscal_numero_ncm='" . trim($this->mgt_classificacao_fiscal_numero_ncm->Text) . "'";
   ...
   function cadncmstabelasaltJSSubmit($sender, $params)
   {
...
<?php
   ...
   function cadncmstabelasaltJSLoad($sender, $params)
   {
...
global $cadncmstabelasalt;

//Creates the form
$cadncmstabelasalt = new cadncmstabelasalt($application);

//Read from resource file
$cadncmstabelasalt->loadResource(__FILE__);

//Shows the form
$cadncmstabelasalt->show();
