...
//Definicao de classe
class relcomissoesimp extends Page
{
   ...
   public $Label9 = null;
   public $mgt_swap_comissao_pagamento = null;
   public $Label8 = null;
   ...
   public $mgt_swap_comissao_vencimento = null;
   ...
   function relcomissoesimpCreate($sender, $params)
   {
      ...
      for($ind = 1; $ind <= 12; $ind++)
      {
         if(trim($cfops) <> '')
         {
            ...
            $Comando_SQL .= "(";
            $Comando_SQL .= "(mgt_nota_fiscal_dup_dt_vcto_" . $ind . " >= '" . inverte_data_dma_to_amd(trim($this->data_inicial->Caption)) . "') And ";
            $Comando_SQL .= "(mgt_nota_fiscal_dup_dt_vcto_" . $ind . " <= '" . inverte_data_dma_to_amd(trim($this->data_final->Caption)) . "')";
            $Comando_SQL .= ") and ";
         ...
         if((GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->EOF) != 1)
         {
            ...
            {
               if(trim(GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->Fields['mgt_nota_fiscal_dup_nro_' . $ind]) != '')
               {
                  ...
                  $Comando_SQL .= "mgt_swap_comissao_pagamento, ";
				  ...
				  if($ind == 1)
				  {
                    $Comando_SQL .= "'" . (GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->Fields['mgt_nota_fiscal_dup_vlr_' . $ind] - GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->Fields['mgt_nota_fiscal_valor_icms_sub']) . "',";
                  }
				  else
   			      {
                  ...
                  if((GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->Fields['mgt_nota_fiscal_dup_dt_pgto_' . $ind] == '0000-00-00')or(GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->Fields['mgt_nota_fiscal_dup_dt_pgto_' . $ind] == ''))
                  {
                     $Comando_SQL .= "'0.00')";
                  }
                  else
                  {
	 			     if($ind == 1)
  				     {
                        $Comando_SQL .= "'" . (((GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->Fields['mgt_nota_fiscal_dup_vlr_' . $ind] - GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->Fields['mgt_nota_fiscal_valor_icms_sub']) * GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->Fields['mgt_representante_1_porcentagem']) / 100) . "')";
                     }
					 else
					 {
  					    $Comando_SQL .= "'" . ((GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->Fields['mgt_nota_fiscal_dup_vlr_' . $ind] * GetConexaoPrincipal()->SQL_MGT_Notas_Fiscais->Fields['mgt_representante_1_porcentagem']) / 100) . "')";
					 }
                  }
      ...
      for($ind = 1; $ind <= 12; $ind++)
      {
         if(trim($cfops) <> '')
         {
            ...
            $Comando_SQL .= "(";
            $Comando_SQL .= "(mgt_programada_dup_dt_vcto_" . $ind . " >= '" . inverte_data_dma_to_amd(trim($this->data_inicial->Caption)) . "') And ";
            $Comando_SQL .= "(mgt_programada_dup_dt_vcto_" . $ind . " <= '" . inverte_data_dma_to_amd(trim($this->data_final->Caption)) . "')";
            $Comando_SQL .= ") and ";
         ...
         if((GetConexaoPrincipal()->SQL_MGT_Programadas->EOF) != 1)
         {
            ...
            {
               if(trim(GetConexaoPrincipal()->SQL_MGT_Programadas->Fields['mgt_programada_dup_nro_' . $ind]) != '')
               {
                  $Comando_SQL = "Insert into mgt_swap_comissoes(";
                  $Comando_SQL .= "mgt_swap_comissao_nro_duplicata, ";
                  $Comando_SQL .= "mgt_swap_comissao_data, ";
                  ...
                  if((GetConexaoPrincipal()->SQL_MGT_Programadas->Fields['mgt_programada_dup_dt_pgto_' . $ind] == '0000-00-00')or(GetConexaoPrincipal()->SQL_MGT_Programadas->Fields['mgt_programada_dup_dt_pgto_' . $ind] == ''))
                  {
                     $Comando_SQL .= "'0.00')";
                  }
                  else
                  {
                     $Comando_SQL .= "'" . ((GetConexaoPrincipal()->SQL_MGT_Programadas->Fields['mgt_programada_dup_vlr_' . $ind] * GetConexaoPrincipal()->SQL_MGT_Programadas->Fields['mgt_representante_1_porcentagem']) / 100) . "')";
                  }
      ...
      $Comando_SQL = "select *, date_format(mgt_swap_comissao_data, '%d/%m/%Y') AS mgt_swap_comissao_data, date_format(mgt_swap_comissao_vencimento, '%d/%m/%Y') AS mgt_swap_comissao_vencimento, date_format(mgt_swap_comissao_pagamento, '%d/%m/%Y') AS mgt_swap_comissao_pagamento from mgt_swap_comissoes order by mgt_swap_comissao_numero";

      GetConexaoPrincipal()->SQL_MGT_Comissoes->Close();
      GetConexaoPrincipal()->SQL_MGT_Comissoes->SQL = $Comando_SQL;
      GetConexaoPrincipal()->SQL_MGT_Comissoes->Open();
   }
   function relcomissoesimpJSLoad($sender, $params)
   {
...
global $relcomissoesimp;

//Cria o formulario
$relcomissoesimp = new relcomissoesimp($application);

//Ler do arquivo de recursos
$relcomissoesimp->loadResource(__FILE__);

//Mostrar o formulario
$relcomissoesimp->show();
