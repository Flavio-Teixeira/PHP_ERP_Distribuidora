...
?>
   ...
   {
     document.venvendas.bt_cadastro.focus();
     return false;
   }
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.venvendas.bt_buscar.focus();
     return false;
   }
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.venvendas.opcao_busca.focus();
     return false;
   }
   ...
   function bt_cadastroClick($sender, $params)
   {
      ...
      else
      {
         ...
         {
            redirect('ven_vendas_alt.php?mgt_faturamento_numero=' . $this->abrir_cadastro->Text );
         }
      }
   }
...
?>
   ...
   document.venvendas.abrir_cadastro.value = registros.getTableModel().getValue(0, registros.getFocusedRow());
   ...
   function bt_incluirClick($sender, $params)
   {
      ...
      $Comando_SQL = "select *,date_format(mgt_faturamento_data, '%d/%m/%Y') AS mgt_faturamento_data,date_format(mgt_faturamento_data_entrega, '%d/%m/%Y') AS mgt_faturamento_data_entrega, IF(mgt_faturamento_status = 'Preparando','#','') AS mgt_faturamento_status_1, IF(mgt_faturamento_status = 'Faturado','#','') AS mgt_faturamento_status_2, IF(mgt_faturamento_status = 'Aguardando','#','') AS mgt_faturamento_status_3 from mgt_faturamentos order by mgt_faturamento_numero Desc";
      ...
      redirect('ven_vendas_inc.php?mgt_faturamento_numero='.$mgt_faturamento_numero);
   }
   ...
   function bt_buscarClick($sender, $params)
   {
      ...
      {
         $Comando_SQL = "select *,date_format(mgt_faturamento_data, '%d/%m/%Y') AS mgt_faturamento_data,date_format(mgt_faturamento_data_entrega, '%d/%m/%Y') AS mgt_faturamento_data_entrega, IF(mgt_faturamento_status = 'Preparando','#','') AS mgt_faturamento_status_1, IF(mgt_faturamento_status = 'Faturado','#','') AS mgt_faturamento_status_2, IF(mgt_faturamento_status = 'Aguardando','#','') AS mgt_faturamento_status_3 from mgt_faturamentos order by mgt_faturamento_numero";
      }
      else
      {
         if(trim($this->opcao_busca->ItemIndex) == "Numero")
         {
            $Comando_SQL = "select *,date_format(mgt_faturamento_data, '%d/%m/%Y') AS mgt_faturamento_data,date_format(mgt_faturamento_data_entrega, '%d/%m/%Y') AS mgt_faturamento_data_entrega, IF(mgt_faturamento_status = 'Preparando','#','') AS mgt_faturamento_status_1, IF(mgt_faturamento_status = 'Faturado','#','') AS mgt_faturamento_status_2, IF(mgt_faturamento_status = 'Aguardando','#','') AS mgt_faturamento_status_3 from mgt_faturamentos where mgt_faturamento_numero = '" . trim($this->dados_busca->Text) . "' order by mgt_faturamento_numero";
         }
         elseif(trim($this->opcao_busca->ItemIndex) == "Data")
         {
            $Comando_SQL = "select *,date_format(mgt_faturamento_data, '%d/%m/%Y') AS mgt_faturamento_data,date_format(mgt_faturamento_data_entrega, '%d/%m/%Y') AS mgt_faturamento_data_entrega, IF(mgt_faturamento_status = 'Preparando','#','') AS mgt_faturamento_status_1, IF(mgt_faturamento_status = 'Faturado','#','') AS mgt_faturamento_status_2, IF(mgt_faturamento_status = 'Aguardando','#','') AS mgt_faturamento_status_3 from mgt_faturamentos where mgt_faturamento_data = '" . inverte_data_dma_to_amd(trim($this->dados_busca->Text)) . "' order by mgt_faturamento_data";
         }
         elseif(trim($this->opcao_busca->ItemIndex) == "Cliente")
         {
            $Comando_SQL = "select *,date_format(mgt_faturamento_data, '%d/%m/%Y') AS mgt_faturamento_data,date_format(mgt_faturamento_data_entrega, '%d/%m/%Y') AS mgt_faturamento_data_entrega, IF(mgt_faturamento_status = 'Preparando','#','') AS mgt_faturamento_status_1, IF(mgt_faturamento_status = 'Faturado','#','') AS mgt_faturamento_status_2, IF(mgt_faturamento_status = 'Aguardando','#','') AS mgt_faturamento_status_3 from mgt_faturamentos where mgt_faturamento_cliente_nome like '%" . trim($this->dados_busca->Text) . "%' order by mgt_faturamento_cliente_nome";
         }
         elseif(trim($this->opcao_busca->ItemIndex) == "Valor")
         {
            $Comando_SQL = "select *,date_format(mgt_faturamento_data, '%d/%m/%Y') AS mgt_faturamento_data,date_format(mgt_faturamento_data_entrega, '%d/%m/%Y') AS mgt_faturamento_data_entrega, IF(mgt_faturamento_status = 'Preparando','#','') AS mgt_faturamento_status_1, IF(mgt_faturamento_status = 'Faturado','#','') AS mgt_faturamento_status_2, IF(mgt_faturamento_status = 'Aguardando','#','') AS mgt_faturamento_status_3 from mgt_faturamentos where mgt_faturamento_valor_total = '" . trim($this->dados_busca->Text) . "' order by mgt_faturamento_valor_total";
         }
         elseif(trim($this->opcao_busca->ItemIndex) == "Entrega")
         {
            $Comando_SQL = "select *,date_format(mgt_faturamento_data, '%d/%m/%Y') AS mgt_faturamento_data,date_format(mgt_faturamento_data_entrega, '%d/%m/%Y') AS mgt_faturamento_data_entrega, IF(mgt_faturamento_status = 'Preparando','#','') AS mgt_faturamento_status_1, IF(mgt_faturamento_status = 'Faturado','#','') AS mgt_faturamento_status_2, IF(mgt_faturamento_status = 'Aguardando','#','') AS mgt_faturamento_status_3 from mgt_faturamentos where mgt_faturamento_data_entrega = '" . inverte_data_dma_to_amd(trim($this->dados_busca->Text)) . "' order by mgt_faturamento_data_entrega";
         }
         elseif(trim($this->opcao_busca->ItemIndex) == "Status")
         {
            $Comando_SQL = "select *,date_format(mgt_faturamento_data, '%d/%m/%Y') AS mgt_faturamento_data,date_format(mgt_faturamento_data_entrega, '%d/%m/%Y') AS mgt_faturamento_data_entrega, IF(mgt_faturamento_status = 'Preparando','#','') AS mgt_faturamento_status_1, IF(mgt_faturamento_status = 'Faturado','#','') AS mgt_faturamento_status_2, IF(mgt_faturamento_status = 'Aguardando','#','') AS mgt_faturamento_status_3 from mgt_faturamentos where mgt_faturamento_status = '" . trim($this->dados_busca->Text) . "' order by mgt_faturamento_status";
         }
      }
   ...
   function venvendasCreate($sender, $params)
   {
   ...
   function venvendasJSLoad($sender, $params)
   {
...
//Creates the form
$venvendas = new venvendas($application);

//Read from resource file
$venvendas->loadResource(__FILE__);
