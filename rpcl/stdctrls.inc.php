        ...
        function dumpHeaderCode()
        {
                if (!defined('DYNAPI'))
                {
                        echo "<script type=\"text/javascript\" src=\""
                           . RPCL_HTTP_PATH . "/dynapi/src/dynapi.js\"></script>\n"
                           . "<script type=\"text/javascript\">\n"
                           . "  dynapi.library.setPath('" . RPCL_HTTP_PATH . "/dynapi/src/');\n"
...
function __QLibrary_InitLib()
{
        if (!defined('QOOXDOO'))
        {
                echo "<script type=\"text/javascript\" src=\"" . RPCL_HTTP_PATH . "/qooxdoo/framework/script/qx.js\" charset=\"UTF-8\"></script>\n"
                   . "<script type=\"text/javascript\">\n"
                   . "  qx.log.Logger.ROOT_LOGGER.setMinLevel(qx.log.Logger.LEVEL_FATAL);\n"
                   . "  qx.manager.object.AliasManager.getInstance().add(\"static\", \"" . RPCL_HTTP_PATH . "/qooxdoo/framework/resource/static/\");\n"
                   . "  qx.manager.object.AliasManager.getInstance().add(\"widget\", \"" . RPCL_HTTP_PATH . "/qooxdoo/framework/resource/widget/windows/\");\n"
                   . "  qx.manager.object.AliasManager.getInstance().add(\"icon\", \"" . RPCL_HTTP_PATH . "/qooxdoo/framework/resource/icon/VistaInspirate/\");\n"
                   . "</script>\n";
    ...
    */
    ...
    function defaultFormatAsDate() { return ""; }
        ...
        function dumpContents()
        {
                ...
                $this->openingWrap($style,$alignment,$hint,$events,$class);

                $this->openingLink($style,$alignment,$hint,$target,$events,$class);
    ...
    function defaultFilterInput() { return 1; }
        ...
        */
        protected function getCommonAttributes()
        {
                ...
                return "$disabled $maxlength $readonly $taborder $hint $events $class $extra";
        }
        ...
        */
        ...
        function dumpContents()
        {
                ...
                if($this->_type=="text" && $this->_ispassword)
                  $this->_type="password";
                ...
                echo "<input type=\"$this->_type\" id=\"$this->_name\" onchange=\"return {$this->Name}_updatehidden(event)\" name=\"$this->_name\" value=\"$avalue\" $style $attributes />";
...
class CustomListBox extends CustomMultiSelectListControl
{
        ...
        function dumpContents()
        {
                ...
                echo "<select name=\"$name\" id=\"$this->_name\" size=\"$this->_size\" $style $enabled $multiselect $taborder $hint $events $attributes $class>";
...
class ButtonControl extends FocusControl
{
        ...
        function init()
        {
                ...
                if (($submitEvent=='') && ($fbapplication!=null))
                {
        ...
        */
          ...
          $additionalAttributes = "", $surroundingTags = "%s", $composite = false)
        {
            ...
            $additionalAttributes.=$this->strAttributes();
                ...
                $input = "<input type=\"$inputType\" name=\"$name\" value=\"$avalue\" $events $style $checked $enabled $taborder $hint $additionalAttributes $class />";
                // output the control
...
class CustomCheckBox extends ButtonControl
{
        ...
        function dumpContents()
        {
                ...
                $label = "<label id=\"{$this->_name}_caption\" $hint $style $class for=\"$this->_name\">";
                $label.= $this->_caption;
                $label .= "</label>";

                //if the component is a MRadioButton we just need a cell in the table
                if($MBox)
                    $format = "<tr><td style=\"height:{$this->_height}px\" valign='top' >%s $label</td></tr>\r\n";
...
class RadioButton extends ButtonControl
{
        ...
        function dumpContents()
        {
                ...
                {
                  $surroundingTags = "<table cellpadding=\"0\" cellspacing=\"0\" id=\"{$this->_name}_table\"  $class><tr>";
                  $surroundingTags.= "<td  style=\"height:{$this->_height}px\" valign='top'>%s\n";
                  // Add some JS to the Caption (OnClick).
                  $surroundingTags .= ($this->Owner != null) ? "<label id=\"{$this->_name}_caption\" $hint $style $class for=\"$this->_name\">" : "<label>";
                  $surroundingTags .= $this->_caption;
...
class ScrollBar extends QWidget
{
        ...
        function dumpContents()
        {
                ...
                //Pagesize, minimum and maximum. By the way minimum doesn? exist in
