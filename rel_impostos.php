...
//Class definition
class relimpostos extends Page
{
   public $tipo = null;
...
?>
   ...
   var campo = document.relimpostos.mgt_data_final
...
?>
   ...
   var campo = document.relimpostos.mgt_data_inicial
   var digits="0123456789/"
   var campo_temp
   for (var i=0;i<campo.value.length;i++){
       campo_temp=campo.value.substring(i,i+1)
...
?>
   ...
   {
     document.relimpostos.bt_imprimir.focus();
     return false;
   }
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.relimpostos.mgt_data_final.focus();
     return false;
   }
...
<?php
   ...
   function bt_imprimirClick($sender, $params)
   {
      ...
      if(trim($this->MSG_Erro->Caption) == '')
      {
         ...
         $_SESSION['imprime_tipo'] = $this->tipo->Value;

         echo "<script language=\"JavaScript\">";
         echo "window.open('rel_impostos_imp.php','rel_impostos_imp','toolbar=yes,location=no,directories=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,maximized=no,fullscreen=no,width=775,innerWidth=775,offsetWidth=775,clientWidth=775,pageWidth=775,height=550,innerHeight=550,offsetHeight=550,clientHeight=550,pageHeight=550');";
         echo "</script>";
      }
   }

   function relimpostosCreate($sender, $params)
   {
      ...
      $this->tipo->Value = $_REQUEST['tipo'];
      $tipo = $_REQUEST['tipo'];

      if($tipo == "PIS")
      {
         $this->area_sistema->Caption = "RELATORIO DE IMPOSTOS - PIS";
      }
      elseif($tipo == "COFINS")
      {
         $this->area_sistema->Caption = "RELATORIO DE IMPOSTOS - COFINS";
      }
      elseif($tipo == "ICMS")
      {
         $this->area_sistema->Caption = "RELATORIO DE IMPOSTOS - ICMS";
      }
      elseif($tipo == "CSLL")
      {
         $this->area_sistema->Caption = "RELATORIO DE IMPOSTOS - CSLL";
      }
      elseif($tipo == "IRPJ")
      {
         $this->area_sistema->Caption = "RELATORIO DE IMPOSTOS - IRPJ";
      }
      elseif($tipo == "IPI")
      {
         $this->area_sistema->Caption = "RELATORIO DE IMPOSTOS - IPI";
      }
   ...
   function relimpostosJSLoad($sender, $params)
   {
...
//Creates the form
$relimpostos = new relimpostos($application);

//Read from resource file
$relimpostos->loadResource(__FILE__);
