...
<?php
   ...
   {
      //********************************************************
      //*** Funções Utilizadas pela Rotina da Geração da XML ***
      //********************************************************
      ...
      //*** a opção abaixo desativa este processo           ***
      //*******************************************************
      ...
      $tipo_ambiente = $_SESSION['login_nfe_ambiente'];// 1 - Produção || 2 - Homologação
      $serie_nfe = $_SESSION['login_nfe_serie'];// Série da Nota Fiscal
      $cnpj_padrao = completa_zeros_retira_caracter($_SESSION['login_cnpj'], 14);
      $senha_nfe = $_SESSION['login_nfe_senha'];
      $status_servico = false;

      //*** Obtem o Número da Nota Fiscal Eletrônica ***
      $mgt_numero_nfe = $this->mgt_nota_fiscal_numero->Text;

      //*** Verifica o Status do Serviço no Servidor de SOAP ***
      $status_servico_SOAP = status_servidor_SOAP($tipo_ambiente);
      $status_servico = $status_servico_SOAP["status"];

      if(!$status_servico)
      {
         if(($this->hd_nota_fiscal_nao_imprimir->Value == 0)and($this->hd_apenas_registra->Value == 0))
         {
            $listagem_erros = 'ATENÇÃO: ' . trim($status_servico_SOAP["mensagem"]);
      ...
      //******************************************************************
      //*** INICIO - Gerando o Envio de Lote da Nota Fiscal Eletrônica ***
      //******************************************************************
      ...
      if($status_servico)
      {
         ...
         if($validacao["status"] == '1')
         {
            ...
            if($tipo_ambiente == 1)
            {
               $servidor_wsdl = 'https://nfe.fazenda.sp.gov.br/ws/nfeautorizacao.asmx';
            }
            else
            {
               $servidor_wsdl = 'https://homologacao.nfe.fazenda.sp.gov.br/ws/nfeautorizacao.asmx';
            }
            ...
            $envio_xml = simplexml_load_file('nfe/saida/enviNFe_' . trim($mgt_numero_nfe) . '_sign.xml');
            $cabecalho = '<nfeCabecMsg xmlns="http://www.portalfiscal.inf.br/nfe/wsdl/NfeAutorizacao"><cUF>35</cUF><versaoDados>3.10</versaoDados></nfeCabecMsg>';
            $dados = '<nfeDadosMsg xmlns="http://www.portalfiscal.inf.br/nfe/wsdl/NfeAutorizacao">' . trim(str_replace('<?xml version="1.0" encoding="UTF-8"?>', '', $envio_xml->asXML())) . '</nfeDadosMsg>';
            ...
            $grava_retorno_xml = fopen('nfe/retEnviNFe.xml', "w");
            ...
            $ler_retorno_nfe_xml = simplexml_load_file('nfe/retEnviNFe.xml');
            $MSG_Erro = utf8_decode($ler_retorno_nfe_xml->retEnviNFe->xMotivo);
            $nro_recebimento = utf8_decode($ler_retorno_nfe_xml->retEnviNFe->infRec->nRec);

            if(trim($MSG_Erro) == 'Lote recebido com sucesso')
            {
               //***********************************
               //*** INICIO - Arquivo de Retorno ***
               //***********************************

               //*** INICIO - Validando o XML (Schema consReciNFe_v3.10.xsd) ***

               //*** versao do encoding xml ***
               $geraDomRet = new DOMDocument('1.0', 'UTF-8');

               //*** retirar os espacos em branco ***
               $geraDomRet->preserveWhiteSpace = false;
               //*** gerar o codigo ***
               $geraDomRet->formatOutput = false;// Se True Prevalece a Formatação com Estrutura (Pulando Linha e Espaços).

               //*** criando o nó principal (Raiz) ***
               //*** Cabeça do XML ***

               $bp01 = $geraDomRet->createElement('consReciNFe');
               //*** Adiciona Atributo BP02 ***
               $bp01->setAttribute('xmlns', 'http://www.portalfiscal.inf.br/nfe');
               $bp01->setAttribute('versao', '3.10');
               $bp03 = $geraDomRet->createElement('tpAmb', $tipo_ambiente);
               $bp04 = $geraDomRet->createElement('nRec', $nro_recebimento);

               $bp01->appendChild($bp03);
               $bp01->appendChild($bp04);

               $geraDomRet->appendChild($bp01);

               //*** Salvando o XML ***
               $geraDomRet->save('nfe/consReciNFe.xml');

               //*** Verifica a Validação do Schema ***
               $validacao = validaXML('nfe/consReciNFe.xml', 'validadores/consReciNFe_v3.10.xsd');
               ...
               if($validacao["status"] == '1')
               {
                  ...
                  while($verifica_novamente == true)
                  {
                     ...
                     if($tipo_ambiente == 1)
                     {
                        $servidor_wsdl = 'https://nfe.fazenda.sp.gov.br/ws/nferetautorizacao.asmx';
                     }
                     else
                     {
                        $servidor_wsdl = 'https://homologacao.nfe.fazenda.sp.gov.br/ws/nferetautorizacao.asmx';
                     }
                     ...
                     $envio_xml = simplexml_load_file('nfe/consReciNFe.xml');
                     $cabecalho = '<nfeCabecMsg xmlns="http://www.portalfiscal.inf.br/nfe/wsdl/NfeRetAutorizacao"><cUF>35</cUF><versaoDados>3.10</versaoDados></nfeCabecMsg>';
                     $dados = '<nfeDadosMsg xmlns="http://www.portalfiscal.inf.br/nfe/wsdl/NfeRetAutorizacao">' . trim(str_replace('<?xml version="1.0" encoding="UTF-8"?>', '', $envio_xml->asXML())) . '</nfeDadosMsg>';
                     ...
                     $retorno_xml = str_replace('&lt;', '<', $retorno_xml);
                     $retorno_xml = str_replace('&gt;', '>', $retorno_xml);
                     $retorno_xml = str_replace('<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema"><soap:Header><nfeCabecMsg xmlns="http://www.portalfiscal.inf.br/nfe/wsdl/NfeRetAutorizacao"><cUF>35</cUF><versaoDados>3.10</versaoDados></nfeCabecMsg></soap:Header><soap:Body>', '', $retorno_xml);
                     $retorno_xml = str_replace('</soap:Body></soap:Envelope>', '', $retorno_xml);
                     $retorno_xml = str_replace('utf-8', 'UTF-8', $retorno_xml);

                     //*** Grava o XML de Retorno ***
                     $grava_retorno_xml = fopen('nfe/RetConsReciNFe.xml', "w");
                     fwrite($grava_retorno_xml, $retorno_xml);
                     fclose($grava_retorno_xml);

                     //*** Lê o Retorno do XML e Verifica o Status ***
                     $ler_retorno_nferet_xml = simplexml_load_file('nfe/RetConsReciNFe.xml');

                     $MSG_Erro = utf8_decode($ler_retorno_nferet_xml->retConsReciNFe->protNFe->infProt->xMotivo);

                     if(trim($MSG_Erro) == '')
                     {
                        $MSG_Erro = utf8_decode($ler_retorno_nferet_xml->retConsReciNFe->xMotivo);
                     }

                     $nroNFe = utf8_decode($ler_retorno_nferet_xml->retConsReciNFe->protNFe->infProt->chNFe);
                     $nroNFe = substr(trim($nroNFe), (strlen(trim($nroNFe)) - 8), 7);
                     $chNFe = utf8_decode($ler_retorno_nferet_xml->retConsReciNFe->protNFe->infProt->chNFe);
                     $nProt = utf8_decode($ler_retorno_nferet_xml->retConsReciNFe->protNFe->infProt->nProt);

                     //*** Caso haja erros utilizar rotina abaixo ***
                     //print_r($ler_retorno_nferet_xml);

                     if(trim($MSG_Erro) == 'Autorizado o uso da NF-e')
                     {
                        $listagem_erros = 'NOTA FISCAL GERADA!!!\n\n\n\n';
                        $listagem_erros .= 'Nota Fiscal Eletronica Registrada com Sucesso\n\n';
                        $listagem_erros .= ' || Nro.NFe: ' . $nroNFe . '\n\n';
                        $listagem_erros .= ' || Nro.Recebimento: ' . $nro_recebimento . '\n\n';
                        $listagem_erros .= ' || Nro.Protocolo: ' . $nProt . '\n\n';
                        $listagem_erros .= ' || Nro.Chave de Acesso: ' . $chNFe . '\n\n';
                        echo "<script language=\"JavaScript\">alert('#2: " . $listagem_erros . "');</script>";
                        $this->confirmacao->Top = 825;
                        $this->confirmacao->IsVisible = false;
                        $this->hd_bt_sim_clicado->Value = 0;

                        //*** INICIO - Prepara a Geração do Arquivo de Envio para o Cliente ***

                        copy('nfe/saida/enviNFe_' . trim($mgt_numero_nfe) . '_sign.xml', 'swap/swap_NFe_' . trim($mgt_numero_nfe) . '.xml');

                        //*** Retira as itens desnecessários para a geração ***

                        //*** Lê a Nota Fiscal Eletrônica ***
                        ...
                        $nome_arquivo_cliente = 'nfe/RetConsReciNFe.xml';
                        $arquivo_cliente = fopen($nome_arquivo_cliente, 'r');
                        $conteudo_ret_cliente = fread($arquivo_cliente, filesize($nome_arquivo_cliente));
                        fclose($arquivo_cliente);

                        $posicao_inicial = strpos($conteudo_ret_cliente, '<protNFe');
                        $posicao_final = strpos($conteudo_ret_cliente, '</protNFe>');
                        $tamanho_total = strlen($conteudo_ret_cliente);

                        $conteudo_ret_gravar = trim(substr($conteudo_ret_cliente, $posicao_inicial, (($posicao_final - $posicao_inicial) + 10)));
                        $conteudo_ret_gravar = str_replace('<protNFe versao="3.10">', '<protNFe versao="3.10" xmlns="http://www.portalfiscal.inf.br/nfe">', $conteudo_ret_gravar);
                        ...
                        //*** FINAL - Prepara a Geração do Arquivo de Envio para o Cliente ***
                        ...
                        if((GetConexaoPrincipal()->SQL_MGT_Faturamentos_Produtos->EOF) != 1)
                        {
                           while((GetConexaoPrincipal()->SQL_MGT_Faturamentos_Produtos->EOF) != 1)
                           {
                              ...
                              if($_SESSION['login_estoque'] == "S")
                              {
                                 ...
                                 $Comando_SQL .= " WHERE ";
                                 $Comando_SQL .= "Produto_Cod_Barra = '" . trim(GetConexaoPrincipal()->SQL_MGT_Faturamentos_Produtos->Fields['mgt_faturamento_produto_codigo_barras']) . "'";
                        ...
                        //*** Atualiza o Número da Última Nota Fiscal ***
                        ...
                        //*** Limpa os Campos para o Próximo Faturamento ***
                        ...
                        echo "window.open('mgt_nfe_danfe.php?nfe=" . "NFe_" . trim($mgt_numero_nfe) . ".xml','NFe_" . trim($mgt_numero_nfe) . "','toolbar=yes,location=no,directories=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,maximized=no,fullscreen=no,width=775,innerWidth=775,offsetWidth=775,clientWidth=775,pageWidth=775,height=550,innerHeight=550,offsetHeight=550,clientHeight=550,pageHeight=550');";
                        echo 'window.location="nf_emissao_nota.php";';
                        echo "</script>";
                     }
                     else
                     {
                        if(trim($MSG_Erro) == 'Lote em processamento')
                        {
                           $listagem_erros = 'Lote em processamento: O Sistema Transmitirá Novamente, clique em OK.';
                        ...
                        else if(trim($MSG_Erro) == 'Rejeição: Duplicidade de NF-e')
                        {
                           $listagem_erros = 'Rejeição - Duplicidade de NF-e: Por favor, altere a numeração da Última Nota Emitida e Tente enviar novamente.';
                        ...
                        {
                           $listagem_erros = 'ATENÇÃO: Foram Encontrados Erros no XML: \n\n' . $MSG_Erro . '. Corrija o Problema e Tente enviar novamente.';
               ...
               else
               {
                  ...
                  $listagem_erros = 'Problemas Encontrados: ' . $exibe_erro;
            ...
            else
            {
               ...
               $listagem_erros = 'Problemas Encontrados: ' . $exibe_erro;
               echo "<script language=\"JavaScript\">alert('#7: " . $listagem_erros . "');</script>";
               $this->confirmacao->Top = 825;
               $this->confirmacao->IsVisible = false;
               $this->hd_bt_sim_clicado->Value = 0;
            }
      ...
      else
      {
         if($this->hd_nota_fiscal_nao_imprimir->Value <> 0)
         {
            ...
            $Comando_SQL = "update mgt_faturamentos set ";
            $Comando_SQL .= "mgt_faturamento_status = 'Contingência' ";
            $Comando_SQL .= "Where mgt_faturamento_numero = '" . trim($this->mgt_nota_fiscal_faturamento->Text) . "'";
            ...
            copy('nfe/saida/enviNFe_' . trim($mgt_numero_nfe) . '_sign.xml', 'nfe/enviado/NFe_' . trim($mgt_numero_nfe) . '_Contingencia.xml');
			copy('nfe/saida/enviNFe_' . trim($mgt_numero_nfe) . '_sign.xml', 'nfe/entregar_cliente/NFe_' . trim($mgt_numero_nfe) . '.xml');
            ...
            //*** Abre a Janela do DANFE EM CONTINGENCIA ***

            echo "<script language=\"JavaScript\">";
            echo "var pos_top = (parseInt((screen.height)/2) - 320);";
            echo "var pos_left = (parseInt((screen.width)/2) - 387);";
            echo "window.open('mgt_nfe_danfe.php?nfe=" . "NFe_" . trim($mgt_numero_nfe) . ".xml','NFe_" . trim($mgt_numero_nfe) . "_Contingencia','toolbar=yes,location=no,directories=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,maximized=no,fullscreen=no,width=775,innerWidth=775,offsetWidth=775,clientWidth=775,pageWidth=775,height=550,innerHeight=550,offsetHeight=550,clientHeight=550,pageHeight=550');";
            echo 'window.location="nf_emissao_nota.php";';
            echo "</script>";
         }
      }
      ...
      //*****************************************************************
      //*** FINAL - Gerando o Envio de Lote da Nota Fiscal Eletrônica ***
      //*****************************************************************
   }
   ...
   public function gerar_xml()
   {
      ...
      //*** INICIO - Obtem o Número da Nota Fiscal Eletrônica ***

      $mgt_numero_nfe = $this->mgt_nota_fiscal_numero->Text;

      //*** Apaga os Arquivos já gerados ***
      ...
      //*** FINAL - Obtem o Número da Nota Fiscal Eletrônica ***
      ...
      //*** gerar o codigo ***
      $geraDom->formatOutput = false;// Se True Prevalece a Formatação com Estrutura (Pulando Linha e Espaços).
      ...
      $b11 = $geraDom->createElement('tpNF', trim($this->mgt_nota_fiscal_tipo_nota->ItemIndex));//*** 1 - Nota de Saída || 0 - Nota de Entrada ***
      ...
      //*** Endereço do Emissor da Nota Fiscal ***
      ...
      else
      {
		...
		{
			$e02 = $geraDom->createElement('idEstrangeiro', completa_zeros_retira_caracter($cliente_codigo, 14));
		}
		else
		{
      ...
      //*** Endereço do Destinatário da Nota Fiscal ***
      ...
      else
      {
		 if(trim($this->mgt_nota_fiscal_exportacao_uf_emb->Text) <> '')
		 {
			$e17a = $geraDom->createElement('indIEDest', '9');		 
		 }
		 else
		 {
			$e17a = $geraDom->createElement('indIEDest', '2');		 
      ...
      while((GetConexaoPrincipal()->SQL_MGT_Faturamentos_Produtos->EOF) <> 1)
      {
         ...
         //{
         //   $i03 = $geraDom->createElement('cEAN', completa_zeros_retira_caracter(trim(GetConexaoPrincipal()->SQL_MGT_Produtos->Fields['mgt_produto_codigo_barras']), 13) );
         //}
         //else
         //{
         ...
         //if(GetConexaoPrincipal()->SQL_MGT_Produtos->Fields['mgt_produto_codigo_barras'] > 0)
         //{
         //   $i12 = $geraDom->createElement('cEANTrib', completa_zeros_retira_caracter(trim(GetConexaoPrincipal()->SQL_MGT_Produtos->Fields['mgt_produto_codigo_barras']), 13) );
         //}
         //else
         //{
      ...
      //*** Registra a Informação da Fatura ***

      if(trim($this->hd_nota_fiscal_descricao_condicao_pgto->Value) == '1')
      {
         $informacoes_adicionais = 'CONDICAO DE PAGTO: (À VISTA) // ';
      }
      ...
      //*** Registra as Informações Adicionais ***
      ...
      //*** Prepara a Exibição das Informações Adicionais ***
      ...
      if(trim($this->mgt_nota_fiscal_exportacao_uf_emb->Text) <> '')
      {
         ...
         $za02 = $geraDom->createElement('UFSaidaPais', retira_acentos($this->mgt_nota_fiscal_exportacao_uf_emb->Text, 0));
         $za03 = $geraDom->createElement('xLocExporta', retira_acentos($this->mgt_nota_fiscal_exportacao_local_emb->Text, 0));
		 $za03a = $geraDom->createElement('xLocDespacho', retira_acentos($this->mgt_nota_fiscal_exportacao_local_emb->Text, 0));

         $za01->appendChild($za02);
         $za01->appendChild($za03);
		 $za01->appendChild($za03a);
   ...
   {
      //*** Efetua a Gravação do Arquivo Texto para a Impressão da Nota ***
      ...
      if(trim($this->mgt_nota_fiscal_dup_nro_1->Text) != '')
      {
         ...
         if(trim($this->hd_nota_fiscal_descricao_condicao_pgto->Value) == '1')
         {
            $linha = $this->gera_posicao(1, '(À VISTA)', 0);
         }
         elseif(trim($this->hd_nota_fiscal_descricao_condicao_pgto->Value) == '2')
         {
   ...
   public function imprime_programada()
   {
      ...
      {
         $this->MSG_Erro->Caption = 'Este Número de Venda Programada já existe! Por favor, informe o Número correto.';
      ...
      else
      {
         ...
         //*** Atualiza o Número da Última Venda Programada ***
...
<?php
   ...
   public function obtem_cfop()
   {
      ...
      //*********************************************************
      //*** Gera a Frase de Aproveitamento de Crédito de ICMS ***
      //*********************************************************
...
<?php
   ...
   public function limpa_campos()
   {
      ...
      $this->mgt_nota_fiscal_natureza_operacao->ItemIndex = '--- SELECIONE A NATUREZA DE OPERAÇÃO ---';
      ...
      $this->mgt_nota_fiscal_tipo_transporte->ItemIndex = 'Rodoviário';
...
<?php
      ...
      if(($this->hd_bt_sim_clicado->Value == 0))
      {
         ...
         else
         {
            if($this->hd_nota_fiscal_nao_imprimir->Value == 0)
            {
               ...
               {
                  $this->MSG_Erro->Caption = 'Este Número de Nota Fiscal já existe! Por favor, informe o Número correto.';
               ...
               else
               {
                  ...
                  {
                     //*** Impressão Comum da Nota Fiscal em Matricial ***
                     ...
                     else
                     {
                        ...
                        echo "<script language=\"JavaScript\">alert('#Atenção erros encontrados:\\n\\n\\n" . trim($exibe_erro) . "');</script>";
                     }
   ...
   {
      if(trim($this->mgt_nota_fiscal_natureza_operacao->ItemIndex) != '--- SELECIONE A NATUREZA DE OPERAÇÃO ---')
      {
         //*** Verifica os Endereços de Cobrança e Entrega ***
         ...
         //$this->mgt_nota_fiscal_observacao_nota->Text = 'FAVOR CONFERIR A MERCADORIA NO RECEBIMENTO, NÃO SERÁ ACEITO RECLAMAÇÃO DE FALTA POSTERIOR';
         $this->mgt_nota_fiscal_observacao_nota->Text = '';

         //*** INICIO - Carrega os Cálculos dos Impostos ***
         ...
         //*** FINAL - Carrega os Cálculos dos Impostos ***
         ...
         if(trim($this->mgt_nota_fiscal_suframa->Text) == '')
         {
            ...
            //*** Verifica a Situação Tributária ***
               ...
               if((GetConexaoPrincipal()->SQL_MGT_Faturamentos_Produtos->EOF) != 1)
               {
                  while((GetConexaoPrincipal()->SQL_MGT_Faturamentos_Produtos->EOF) != 1)
                  {
                     ...
                     if((GetConexaoPrincipal()->SQL_MGT_IVA->EOF) != 1)
                     {
                        ...
                        //*** Atualiza a Situação Tributária do Produto ***
            ...
            else
            {
               ...
               if((GetConexaoPrincipal()->SQL_MGT_Faturamentos_Produtos->EOF) != 1)
               {
                  while((GetConexaoPrincipal()->SQL_MGT_Faturamentos_Produtos->EOF) != 1)
                  {
                     ...
                     //*** Seleciona o Produto para Atualizar a Situação Tributária ***
         ...
         else
         {
            ...
            //*** Atualiza os Cálculos do Imposto ***
		 ...
		 $Comando_SQL .= ", mgt_faturamento_produto_codigo_barras "; // *** PDVTEX ***
         ...
         //*** INICIO - Incluir a Legenda na Classificação Fiscal ***
         /*
         ...
         */
         //*** FINAL - Incluir a Legenda na Classificação Fiscal ***
   ...
   function nfemissaonotaimpCreate($sender, $params)
   {
      ...
      //*** INICIO - Carrega a Natureza de Operação ***
      ...
      if((GetConexaoPrincipal()->SQL_MGT_CFOP->EOF) != 1)
      {
         $this->mgt_nota_fiscal_natureza_operacao->AddItem('--- SELECIONE A NATUREZA DE OPERAÇÃO ---', null , '--- SELECIONE A NATUREZA DE OPERAÇÃO ---');
      ...
      //*** FINAL - Carrega a Natureza de Operação ***
   ...
   function bt_imprimirClick($sender, $params)
   {
      if(trim($this->mgt_nota_fiscal_natureza_operacao->ItemIndex) != '--- SELECIONE A NATUREZA DE OPERAÇÃO ---')
      {
         if(trim($this->mgt_nota_fiscal_tipo_faturamento->ItemIndex) == 'Venda Programada')
         {
            $this->msg_confirmacao->Caption = '<center>A Impressora está posicionada corretamente?</center>';
         }
         else
         {
            if($this->hd_apenas_registra->Value == 0)
            {
               $this->msg_confirmacao->Caption = '<center><b>Gerar o XML da NFe</b></center>Você deseja realmente gerar o XML da NFe e Transmitir?<br><font color=#FF0000><b>Obs.: Clique somente uma vez no botão escolhido e aguarde a próxima tela.</b></font>';
            }
            else
            {
               $this->msg_confirmacao->Caption = '<center>Esta opção apenas irá registrar a Nota Fiscal no Banco de Dados do Sistema, deseja continuar?</center>';
            }
         }
      ...
      {
         $this->MSG_Erro->Caption = 'Por favor, selecione a Natureza de Operação.';
      }
   }
   ...
   <?php
    ...
    function mgt_nota_fiscal_tipo_operacaoJSKeyPress($sender, $params)
    {
        ...
        if((event.keyCode == 9) || (event.keyCode == 13) )
        {
          document.nfemissaonotaimp.mgt_nota_fiscal_referenciada.focus();
          return false;
        }
    ...
    function mgt_nota_fiscal_tipo_operacaoJSKeyUp($sender, $params)
    {
        ...
        var campo = document.nfemissaonotaimp.mgt_nota_fiscal_tipo_operacao
        var digits="0123456789"
        var campo_temp
        for (var i=0;i<campo.value.length;i++){
            campo_temp=campo.value.substring(i,i+1)
    ...
    function mgt_nota_fiscal_referenciadaJSKeyUp($sender, $params)
    {
        ...
        var campo = document.nfemissaonotaimp.mgt_nota_fiscal_referenciada
        var digits="0123456789"
        var campo_temp
        for (var i=0;i<campo.value.length;i++){
            campo_temp=campo.value.substring(i,i+1)
