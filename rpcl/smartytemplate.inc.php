        ...
        */
        function headercode()
        {
            if ($this->_headercode=="")
            {
                ...
                ob_end_clean();
                $this->_headercode=$contents.$form->dumpHeaderJavascript(true).$form->dumpChildrenHeaderCode(true);
            }
        ...
        */
        ...
        function assignComponents()
        {
                ...
                while (list($k,$v)=each($form->controls->items))
                {
                        ...
                        {
                            $style='';
                            if ($v->inheritsFrom('QControl')) $style=' style="width:'.$v->Width.'px;height:'.$v->Height.'px" ';
                            $code="<div id=\"".$v->Name."_outer\" $style>\n";
                            $code.=$v->show(true);
                            $code.="\n</div>\n";
        ...
        while (list($k,$v)=each($form->controls->items))
        {
            ...
            if ($dump)
            {
                ...
                $results=preg_match($expr,$tpl_source,$matches);
	                 ...
	                 //IE8 control moves the id to the last attribute, so the reg expression must be modified
                if (count($matches)<2)
	 	            {
	 	              $expr='/<'.strtolower($v->classname()).':php[\s\w\/<>=\\\"]([^<>]*)id="'.$v->Name.'"(.*)>(.*?)<\/'.strtolower($v->classname()).':php>/sm';
	 	              $results=preg_match($expr,$tpl_source,$matches);
	 	            }
