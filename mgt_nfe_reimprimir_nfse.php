...
//Definicao de classe
class mgtnfereimprimirnfse extends Page
{
   ...
   var campo = document.mgtnfereimprimirnfse.nfe_chave
   ...
   {
     document.mgtnfereimprimirnfse.bt_enviar.focus();
     return false;
   }
   ...
   function bt_enviarClick($sender, $params)
   {
      ...
      {
         if(file_exists('nfe/entregar_cliente/NFSe_' . Trim($this->nfe_numero_nota->Text) . '.xml'))
         {
            ...
            echo "window.open('mgt_nfe_danfe_srv.php?nfse=" . "NFSe_" . trim($this->nfe_numero_nota->Text) . ".xml','NFSe_" . trim($this->nfe_numero_nota->Text) . "','toolbar=yes,location=no,directories=no,status=no,menubar=yes,scrollbars=yes,resizable=yes,maximized=no,fullscreen=no,width=775,innerWidth=775,offsetWidth=775,clientWidth=775,pageWidth=775,height=550,innerHeight=550,offsetHeight=550,clientHeight=550,pageHeight=550');";
...
?>
   ...
   var campo = document.mgtnfereimprimirnfse.nfe_numero_nota
   var digits="0123456789"
   var campo_temp
   for (var i=0;i<campo.value.length;i++){
       campo_temp=campo.value.substring(i,i+1)
...
?>
   ...
   if((event.keyCode == 9) || (event.keyCode == 13) )
   {
     document.mgtnfereimprimirnfse.nfe_chave.focus();
     return false;
   }
...
<?php
   ...
   function mgtnfereimprimirnfseCreate($sender, $params)
   {
   ...
   function mgtnfereimprimirnfseJSLoad($sender, $params)
   {
...
global $mgtnfereimprimirnfse;

//Cria o formulario
$mgtnfereimprimirnfse = new mgtnfereimprimirnfse($application);

//Ler do arquivo de recursos
$mgtnfereimprimirnfse->loadResource(__FILE__);

//Mostrar o formulario
$mgtnfereimprimirnfse->show();
